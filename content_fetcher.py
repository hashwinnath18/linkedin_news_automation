...
def _safe_load_feed(url):
    """Safely load and parse an RSS feed. Returns feed or None on failure/empty."""
    try:
        feed = feedparser.parse(url)
        # bozo indicates some parsing/network issue; entries might still exist,
        # but for robustness we treat serious errors or empty entries as unusable.
...
def get_article():
    ...
    Tries multiple feeds, falling back to alternates if one is empty/broken.
    """
    if not RSS_FEEDS:
        raise RuntimeError("No RSS feeds configured.")

    # Randomize order so you don't always hit the same feed first
    feeds_to_try = RSS_FEEDS.copy()
    random.shuffle(feeds_to_try)

    feed = None
    chosen_feed_url = None

    # First pass: find any feed that loads and has entries
    for feed_url in feeds_to_try:
        feed = _safe_load_feed(feed_url)
        if feed:
            chosen_feed_url = feed_url
            break

    if not feed:
        raise RuntimeError("All RSS feeds failed or returned no entries.")

    # Try to choose a good entry from this feed
    entry = _choose_recent_entry(feed)

    # If this feed had no suitable entry, try others
    if not entry:
        for feed_url in feeds_to_try:
            if feed_url == chosen_feed_url:
                continue
            alt_feed = _safe_load_feed(feed_url)
            if not alt_feed:
                continue
            entry = _choose_recent_entry(alt_feed)
            if entry:
                feed = alt_feed
                chosen_feed_url = feed_url
                break
    ...
    if not entry:
        raise RuntimeError("No usable entries found across all feeds.")
